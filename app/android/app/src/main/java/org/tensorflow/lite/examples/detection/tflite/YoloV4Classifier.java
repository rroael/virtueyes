...
public class YoloV4Classifier implements Classifier {
    ...
    //config yolov4
    private final int INPUT_SIZE = getSize();

    // Number of threads in the java app
    private static final int NUM_THREADS = 4;
    private static boolean isNNAPI = false; // Disponível em todos os dispositivos com o Android 8.1 (API de nível 27) ou versões mais recentes
    ...
    // config yolov4 tiny
    private static int[] OUTPUT_WIDTH_TINY = new int[]{735, 735}; // 224
    //private static final int[] OUTPUT_WIDTH_TINY = new int[]{960, 960}; // 256
    //private static final int[] OUTPUT_WIDTH_TINY = new int[]{2535, 2535}; // 416
    ...
    private ArrayList<Recognition> getDetectionsForFull(ByteBuffer byteBuffer, Bitmap bitmap) {
        ...
        for (int i = 0; i < gridWidth;i++){
            ...
            if (score > getObjThresh()){
                ...
                detections.add(new Recognition("" + i, labels.get(detectedClass),score,rectF,detectedClass, xPos, yPos, 0));
            }
        }
        return detections;
    }
    ...
    private ArrayList<Recognition> getDetectionsForTiny(ByteBuffer byteBuffer, Bitmap bitmap) {
        if(INPUT_SIZE == 224)
            OUTPUT_WIDTH_TINY = new int[]{735, 735}; //224
        if(INPUT_SIZE == 256)
            OUTPUT_WIDTH_TINY = new int[]{960, 960}; // 256
        if(INPUT_SIZE == 416)
            OUTPUT_WIDTH_TINY = new int[]{2535, 2535}; // 416
        ...
        for (int i = 0; i < gridWidth; i++){
            ...
            if (score > getObjThresh()){
                ...
                //LOGGER.d("FOCAL_LENGTH: " + FOCAL_LENGTH + " , VERTICAL_VIEW_ANGLE: " + VERTICAL_VIEW_ANGLE + " , SENSOR_HEIGHT: " + SENSOR_HEIGHT);
                //float verticalAngleAux = (float) (2 * Math.atan(SENSOR_HEIGHT / (FOCAL_LENGTH * 2)));
                //float distanceA = (float) (SENSOR_HEIGHT * Math.tan(verticalAngleAux));
                //LOGGER.d("distance: " + distanceA);

                ////float horizonalAngle = (float) (2*Math.atan(w/(CameraActivity.LENS_INFO_AVAILABLE_FOCAL_LENGTHS[0]*2)));
                ////float verticalAngle = (float) (2*Math.atan(h/(CameraActivity.LENS_INFO_AVAILABLE_FOCAL_LENGTHS[0]*2)));
                //float verticalAngle = (float) (2*Math.atan(h/(CameraActivity.VERTICAL_ANGLE*2)));
                //float sensorHeight = (float) (Math.tan(verticalAngle/2) * 2 * CameraActivity.FOCAL_LENGTH);

                // d = h * tan(a)
                ////float distance = CameraActivity.FOCAL_LENGTH * sensorHeight / bitmap.getHeight();
                //float distance = (float) (sensorHeight * Math.tan(verticalAngle));
                //LOGGER.d("distance: " + distance + ", sensorHeight: " + sensorHeight + ", VERTICAL_ANGLE: " + verticalAngle + ", H: " + h);

                /*if(CameraActivity.useCamera2API) {
                    if (CameraActivity.LENS_INFO_AVAILABLE_FOCAL_LENGTHS != null) {
                        float[] maxFocus = CameraActivity.LENS_INFO_AVAILABLE_FOCAL_LENGTHS;
                        float wAux = CameraActivity.SENSOR_INFO_PHYSICAL_SIZE.getWidth();
                        float hAux = CameraActivity.SENSOR_INFO_PHYSICAL_SIZE.getHeight();
                        float horizonalAngleAux = (float) (2 * Math.atan(wAux / (maxFocus[0] * 2)));
                        float verticalAngleAux = (float) (2 * Math.atan(hAux / (maxFocus[0] * 2)));

                        float distanceA = (float) (hAux * Math.tan(verticalAngleAux));
                        LOGGER.d("distanceA: " + distanceA + ", hAux: " + hAux + " , verticalAngleAux: " + verticalAngleAux);
                    }
                } else {

                    float distanceA = (float) (CameraActivity.SENSOR_SIZE_HEIGHT * Math.tan(CameraActivity.VERTICAL_ANGLE));
                    LOGGER.d("distanceA: " + distanceA + ", SENSOR_SIZE_HEIGHT: " + CameraActivity.SENSOR_SIZE_HEIGHT + " , VERTICAL_ANGLE: " + CameraActivity.VERTICAL_ANGLE + " , FOCAL_LENGTH: " + CameraActivity.FOCAL_LENGTH);
                }*/
